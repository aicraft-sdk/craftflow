...
def validate_memory_task_completion(data: dict, metadata: dict, mode: dict) -> int:
    ...
    if not subject.startswith("CRAFTFLOW Memory Update:"):
    ...
    if mode.get("taskMetadata") == "block":
        sys.stderr.write(
            "CRAFTFLOW memory-task completion blocked: router-owned memory finalization evidence "
...
def main() -> int:
    ...
    if not subject.startswith("CRAFTFLOW "):
        return 0
    ...
    if mode.get("taskMetadata") == "block":
        sys.stderr.write(
            "CRAFTFLOW task completion blocked: task description is missing metadata lines: "
            + ", ".join(missing)
            + "\n"
        )
        return 2
