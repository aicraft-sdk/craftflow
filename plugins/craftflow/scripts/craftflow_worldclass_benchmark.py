...
V7_REPO = "craftflow-v7.7.0-stable"
V7_TAG = "v7.7.0"
...
DELTA_RULES: tuple[DeltaRule, ...] = (
    ...
    DeltaRule(
        ...
        "hooks",
        (
            ("CRAFTFLOW v10 workflow context", "session context is version-aware"),
...
def build_claim_scoreboard(
    ...
    current_scenarios = {
        item["scenario"]: item for item in bakeoff_map.get("craftflow-current", [])
    }
    stronger = []
    tied = []
    unproven = []
...
def write_structural_benchmark(
    ...
    lines.append(
        "- `Stronger by evidence` is reserved for deep-bakeoff targets where CRAFTFLOW never scores below the reference on the tested trust scenarios."
    )
    lines.append(
...
def build_roadmap(
    ...
    for repo_result in bakeoff_results:
        if repo_result["repo"] == "craftflow-current":
            continue
        for scenario in repo_result["scenarios"]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="CRAFTFLOW world-class benchmark suite")
    parser.add_argument(
        "--refresh",
        action="store_true",
