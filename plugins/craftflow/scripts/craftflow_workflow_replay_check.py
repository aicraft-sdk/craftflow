...
REQUIRED_FIXTURES = (
    ...
    "doubt-verify-confirmed.json",
    "doubt-verify-refuted.json",
    "doubt-verify-theater.json",
    "intent-interview-dispatched.json",
    "intent-interview-skipped.json",
)
...
def validate_artifact_shape(fixture: dict[str, Any]) -> None:
    ...
    require(
        artifact["state_root"] == ".craftflow/state",
        f"{fixture['id']}: state_root must point to state namespace",
    )
...
def check_doubt_verify_confirmed(fixture: dict[str, Any]) -> None:
    contract = fixture["agent_outputs"]["doubt_verifier_contract"]
    require(
        contract["DOUBT_VERDICT"] == "CONFIRMED",
        "doubt-verify-confirmed: verdict must be CONFIRMED",
    )
    require(
        not contract["BLOCKING"],
        "doubt-verify-confirmed: CONFIRMED verdict must have empty BLOCKING",
    )
    require(
        contract["DOUBT_THEATER"] is False,
        "doubt-verify-confirmed: DOUBT_THEATER must be false on CONFIRMED",
    )
    require(
        fixture["expected"]["next_action"] == "advance_to_next_phase",
        "doubt-verify-confirmed: wrong next action",
    )
    require(
        fixture["expected"]["artifact_delta"]["quality"]["convergence_state"] == "stable",
        "doubt-verify-confirmed: convergence_state must remain stable",
    )


def check_doubt_verify_refuted(fixture: dict[str, Any]) -> None:
    contract = fixture["agent_outputs"]["doubt_verifier_contract"]
    require(
        contract["DOUBT_VERDICT"] == "REFUTED",
        "doubt-verify-refuted: verdict must be REFUTED",
    )
    require(
        bool(contract["BLOCKING"]),
        "doubt-verify-refuted: REFUTED verdict must carry at least one BLOCKING entry",
    )
    blocking = contract["BLOCKING"][0]
    for key in ("claim", "verdict", "evidence", "actionable_fix"):
        require(
            key in blocking and bool(blocking[key]),
            f"doubt-verify-refuted: BLOCKING entry missing or empty field: {key}",
        )
    require(
        bool(contract["REFUTED_CLAIMS"]),
        "doubt-verify-refuted: REFUTED_CLAIMS must be non-empty",
    )
    require(
        fixture["expected"]["next_action"] == "create_remfix_from_refuted_claim",
        "doubt-verify-refuted: wrong next action",
    )
    require(
        fixture["expected"]["artifact_delta"]["quality"]["convergence_state"]
        == "needs_iteration",
        "doubt-verify-refuted: convergence_state must degrade to needs_iteration",
    )


def check_doubt_verify_theater(fixture: dict[str, Any]) -> None:
    contract = fixture["agent_outputs"]["doubt_verifier_contract"]
    require(
        contract["DOUBT_VERDICT"] == "DOUBT_THEATER",
        "doubt-verify-theater: verdict must be DOUBT_THEATER",
    )
    require(
        contract["DOUBT_THEATER"] is True,
        "doubt-verify-theater: DOUBT_THEATER flag must be true",
    )
    require(
        bool(contract.get("DOUBT_THEATER_REASON")),
        "doubt-verify-theater: DOUBT_THEATER_REASON must be non-empty",
    )
    require(
        contract["CYCLE"] >= 2,
        "doubt-verify-theater: DOUBT_THEATER requires at least 2 cycles",
    )
    require(
        not contract["BLOCKING"],
        "doubt-verify-theater: BLOCKING must be empty when theater is declared",
    )
    require(
        fixture["expected"]["next_action"] == "escalate_doubt_theater",
        "doubt-verify-theater: wrong next action",
    )
    require(
        fixture["expected"]["artifact_delta"]["quality"]["convergence_state"]
        == "escalation_needed",
        "doubt-verify-theater: convergence_state must be escalation_needed",
    )
    tasks = fixture["relevant_tasks"]
    require(
        len(tasks) >= 2,
        "doubt-verify-theater: fixture must show at least 2 doubt-verify cycles in relevant_tasks",
    )


def check_intent_interview_dispatched(fixture: dict[str, Any]) -> None:
    require(
        fixture["expected"]["interview_skipped"] is False,
        "intent-interview-dispatched: interview must not be skipped",
    )
    require(
        bool(fixture["expected"].get("dispatch_trigger")),
        "intent-interview-dispatched: dispatch_trigger must be present",
    )
    require(
        fixture["expected"]["next_action"] == "dispatch_planner_with_intent_contract",
        "intent-interview-dispatched: wrong next action",
    )
    contract = fixture["agent_outputs"]["intent_interview_contract"]
    for key in ("goal", "non_goals", "constraints", "acceptance_criteria", "open_decisions", "confidence"):
        require(
            key in contract,
            f"intent-interview-dispatched: intent contract missing key {key}",
        )
    require(bool(contract["goal"]), "intent-interview-dispatched: goal must be non-empty")
    require(
        bool(contract["acceptance_criteria"]),
        "intent-interview-dispatched: acceptance_criteria must be non-empty",
    )
    require(
        contract["confidence"] in ("high", "medium", "low"),
        "intent-interview-dispatched: confidence must be high, medium, or low",
    )
    require(
        not contract["open_decisions"],
        "intent-interview-dispatched: open_decisions must be empty for high-confidence interview",
    )
    intent_delta = fixture["expected"]["artifact_delta"]["intent"]
    require(
        intent_delta["goal"] == contract["goal"],
        "intent-interview-dispatched: artifact_delta must persist the interview goal",
    )
    tasks = fixture["relevant_tasks"]
    require("intent_interview" in tasks, "intent-interview-dispatched: missing intent_interview task")
    require(
        tasks["intent_interview"]["phase"] == "plan-intent-interview",
        "intent-interview-dispatched: intent_interview task must use phase plan-intent-interview",
    )


def check_intent_interview_skipped(fixture: dict[str, Any]) -> None:
    require(
        fixture["expected"]["interview_skipped"] is True,
        "intent-interview-skipped: interview must be marked as skipped",
    )
    require(
        fixture["expected"]["skip_reason"] == "auto_proceed",
        "intent-interview-skipped: skip_reason must be auto_proceed",
    )
    require(
        fixture["expected"]["session_settings"]["AUTO_PROCEED"] is True,
        "intent-interview-skipped: session settings must show AUTO_PROCEED=true",
    )
    require(
        fixture["expected"]["next_action"] == "skip_to_brainstorming",
        "intent-interview-skipped: wrong next action",
    )
    require(
        not fixture["agent_outputs"],
        "intent-interview-skipped: agent_outputs must be empty when interview is skipped",
    )
...
CHECKS = {
    ...
    "doubt-verify-confirmed.json": check_doubt_verify_confirmed,
    "doubt-verify-refuted.json": check_doubt_verify_refuted,
    "doubt-verify-theater.json": check_doubt_verify_theater,
    "intent-interview-dispatched.json": check_intent_interview_dispatched,
    "intent-interview-skipped.json": check_intent_interview_skipped,
}
...
def main() -> int:
    ...
    print("craftflow_workflow_replay_check: OK")
