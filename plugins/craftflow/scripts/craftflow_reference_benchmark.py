...
V7_REF = REF_ROOT / "craftflow-v7.7.0-stable"
...
def detect_signals(repo: Path) -> dict[str, bool]:
    ...
    text_hits["versioned_state"] = (
        ".craftflow/state" in corpus or "state_root" in lower
    )
...
def diff_vs_v7() -> dict[str, object]:
    ...
    critical = [
        ...
        if path.startswith("plugins/craftflow/agents/")
        or path.startswith("plugins/craftflow/skills/")
        or path.startswith("plugins/craftflow/scripts/")
        or path.startswith("plugins/craftflow/tests/")
...
def build_report() -> str:
    ...
    current = next((row for row in rows if row["repo"] == "craftflow-v7.7.0-stable"), None)
    current_craftflow: dict[str, Any] = {
        "repo": "craftflow-current",
        "head": repo_head(ROOT),
        "signals": detect_signals(ROOT),
    }
    current_craftflow["score"], current_craftflow["max_score"] = score(current_craftflow["signals"])
    rows.insert(0, current_craftflow)

    v7_delta = diff_vs_v7()

    lines = []
    lines.append("# CRAFTFLOW Benchmark Report")
    lines.append("")
    ...
    lines.append("")
    lines.append(
        f"- Stable baseline: `craftflow-v7.7.0-stable` @ `{current['head'] if current else 'unknown'}`"
    )
    lines.append(f"- Current repo HEAD: `{current_craftflow['head']}`")
    ...
    lines.append("")
    lines.append("## CRAFTFLOW Current Signal Coverage")
    lines.append("")
    for signal in SIGNALS:
        status = "PASS" if current_craftflow["signals"][signal.key] else "MISS"
    ...
    stronger_refs = []
    for row in rows[1:]:
        if current_craftflow["score"] > row["score"]:
            better_than.append(row["repo"])
        elif current_craftflow["score"] == row["score"]:
