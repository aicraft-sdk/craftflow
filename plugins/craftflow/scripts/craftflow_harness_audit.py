...
ROUTER = PLUGIN_ROOT / "skills" / "craftflow-router" / "SKILL.md"
ROUTER_REFERENCES_DIR = PLUGIN_ROOT / "skills" / "craftflow-router" / "references"
...
TASK_COMPLETED_GUARD = PLUGIN_ROOT / "scripts" / "craftflow_task_completed_guard.py"
...
ORCHESTRATION_BIBLE = ROOT / "docs" / "craftflow-orchestration-bible.md"
ORCHESTRATION_LOGIC = ROOT / "docs" / "craftflow-orchestration-logic-analysis.md"
ORCHESTRATION_SAFETY = ROOT / "docs" / "craftflow-orchestration-safety.md"
...
REPLAY_CHECK = PLUGIN_ROOT / "scripts" / "craftflow_workflow_replay_check.py"
LATENCY_AUDIT = PLUGIN_ROOT / "scripts" / "craftflow_latency_audit.py"
LIVE_HARNESS_RUNNER = PLUGIN_ROOT / "scripts" / "craftflow_live_harness_runner.py"
...
LIVE_MANIFEST_BOOTSTRAP = (
    PLUGIN_ROOT / "tests" / "live" / "manifests" / "craftflow-bootstrap.json"
)
...
def main() -> int:
    ...
    if MARKETPLACE_JSON.exists():
        ...
        if not plugins:
            ...
            if plugin_entry.get("source") != "./plugins/craftflow":
                errors.append(
    ...
    for script in (
        ...
        "craftflow_postcompact_context.py",
        "craftflow_subagent_stop_audit.py",
        "craftflow_precompact_state.py",
        "craftflow_stop_persist.py",
        "craftflow_stop_failure_log.py",
        "craftflow_instructions_loaded_audit.py",
    ):
    ...
    for stale in (
        "live in `.craftflow/`",
        "MEMORY (.craftflow/)",
        "WORKFLOW STATE (.craftflow/workflows/)",
        ".craftflow/\n├── activeContext.md",
    ):
        if stale in readme:
            errors.append(
    ...
    if "CRAFTFLOW Orchestration Bible" not in orchestration_bible:
        errors.append("craftflow-orchestration-bible.md appears malformed")
    if "Craftflow Orchestration Logic Analysis" not in orchestration_logic:
        errors.append("craftflow-orchestration-logic-analysis.md appears malformed")
    if "Craftflow Orchestration Safety" not in orchestration_safety:
        errors.append("craftflow-orchestration-safety.md appears malformed")
    if "CRAFTFLOW Agent Contract Registry" not in read(AGENT_CONTRACT_REGISTRY):
    ...
    # Prompt surfaces, fixtures, and hook runtime (craftflow_hooklib.py) must agree on
    ...
    runtime_scripts = sorted(
        ...
        if p.name not in {"__init__.py", "craftflow_harness_audit.py"}
    )
    for script in runtime_scripts:
        body = read(script)
        if ".claude/craftflow" in body or ".claude\" / \"craftflow" in body:
            errors.append(
                f"{script.name} still references the legacy .claude/craftflow state root"
            )

    if errors:
        return fail(errors)

    print("craftflow_harness_audit: OK")
