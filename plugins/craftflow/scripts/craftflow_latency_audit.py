...
RUNTIME_WORKFLOWS = ROOT / ".craftflow" / "v10" / "workflows"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Summarize CRAFTFLOW workflow latency telemetry."
    )
    parser.add_argument(
        "--fixtures",
        action="store_true",
    ...
    if not artifacts:
        print("craftflow_latency_audit: no workflow artifacts found")
        return 0

    print("craftflow_latency_audit")
