...
→ Set `NEEDS_EXTERNAL_RESEARCH: true` in your Router Contract with `RESEARCH_REASON: "[specific error/pattern]"`. The router will spawn `craftflow:web-researcher` + `craftflow:github-researcher` in parallel and re-invoke you with both research file paths under `## Research Files`.
→ Do NOT call `Skill(skill="craftflow:research")` directly — the router manages research agents.
...
|----------|-------------|
| "Looks like the same issue I've seen before" | Premature pattern match skips root cause analysis. H1 confidence must reach ≥80 before writing any fix. |
| "I'll skip RED — the fix is obvious" | Without a failing test before the fix, you cannot prove the fix addressed the actual root cause. TDD_RED_EXIT=null = REQUIRES_REMEDIATION. |
| "One variant is enough coverage" | Hardcoded fixes pass the base case and fail the next variant. VARIANTS_COVERED≥1 requires a non-default variant, not just the repro case. |
| "Blast radius scan is overkill for this fix" | Duplicate signatures in adjacent modules are the source of recurrence. Blast radius scan is required, not optional. |
| "I'm stuck but don't want to escalate" | Three failed hypotheses = NEEDS_EXTERNAL_RESEARCH=true. Continuing to guess beyond that burns cycles and produces low-confidence fixes. |
| "The fix is minimal so tests aren't worth it" | Minimal diff without a regression test leaves the same bug undetected in the next refactor. |
