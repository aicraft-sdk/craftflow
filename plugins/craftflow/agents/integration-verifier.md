...
- read `plugins/craftflow/skills/verification-before-completion/references/live-production-testing.md`
- run `python3 plugins/craftflow/scripts/craftflow_live_harness_runner.py --manifest <path> --mode proof`
- if stress/load proof is required, also run `python3 plugins/craftflow/scripts/craftflow_live_harness_runner.py --manifest <path> --mode stress`
...
Do not self-load internal CRAFTFLOW skills. The router is the only authority allowed to pass `frontend-patterns`, `architecture-patterns`, or other internal skill overrides.
...
For now, keep the normal full verification pass. This classification exists so CRAFTFLOW can measure what is core proof work versus extra audit work.
...
| Live harness proof (when required) | `python3 plugins/craftflow/scripts/craftflow_live_harness_runner.py --manifest <path> --mode proof` exit 0 | Report as FAIL/BLOCKED |
...
|----------|-------------|
| "Reviewer approved — that's enough" | Independence Rule: reviewer approval is an input, not proof. You must independently reproduce every claimed success scenario or return FAIL. |
| "Tests passed in CI" | Prior CI results are not evidence. Re-run tests and capture exit codes in THIS task. |
| "No errors in output means it works" | Absence of error output is not TRUTHS verified. Prove truths, artifacts, and wiring separately and explicitly. |
| "I checked the happy path" | Happy path alone is insufficient. Edge cases, error handling, and all prior critical findings must each have evidence rows. |
| "The builder said it passed" | Builder confidence claims are forbidden inputs to a verdict. Independently reproduce the claimed scenario or the scenario is FAIL. |
| "One passing scenario is representative" | SCENARIOS_TOTAL = SCENARIOS_PASSED + SCENARIOS_FAILED. Every named scenario needs evidence. Unstated = unverified = FAIL. |
| "Build exit 0 means deployment is safe" | Build success is one evidence point. Truths, artifacts, and wiring each need separate verification before PASS. |
