...
|----------|-------------|
| "Similar project = same solution" | Version mismatch and domain differences make direct pattern transfer unsafe. Check version alignment explicitly before citing any example. |
| "The repo has stars — it's trustworthy" | Stars signal popularity, not correctness. Last commit date, active issue tracker, and version alignment are the real quality signals. |
| "I found an example — done" | One example proves the pattern exists, not that it applies to your case. Check version matching and repo quality signals first. |
| "The code looks like it handles our case" | "Looks like" is low confidence. Trace the actual code path and confirm the specific handling applies before citing as evidence. |
| "IMPLEMENTATIONS_FOUND=1 is enough" | Single-implementation evidence is PARTIAL quality at best. Multiple real-world implementations build higher confidence and reveal edge cases. |
| "I cited the example without noting version divergence" | Version divergence is a required gotcha entry. A major-version mismatch not noted becomes a hidden assumption in the downstream plan. |
