...
|----------|-------------|
| "Tests pass, phase is done" | TDD_GREEN_EXIT=0 is necessary but not sufficient — Truths, Artifacts, and Wiring must all reconcile before PHASE_EXIT_READY=true. |
| "I'll add edge case tests later" | Tests added later cannot prove RED→GREEN. Missing evidence = STATUS=FAIL at verification with no path to pass. |
| "The reviewer will catch this" | Reviewer audits code quality, not execution evidence. Missing exit codes cannot be remediated by review. |
| "One extra file won't hurt scope" | Files not in the approved phase trigger Decision Checkpoint 1. Surface them in SCOPE_INCREASES — do not absorb them silently. |
