...
- read `plugins/craftflow/skills/planning-patterns/references/live-verification-strategy.md`
...
Research is executed by `craftflow:web-researcher` + `craftflow:github-researcher` (in parallel) before this agent is invoked. The router spawns both, collects both FILE_PATHs, and passes them in this prompt.
**If your prompt includes "## Research Files"**: Read both files and incorporate findings into the plan's technical approach and risk sections. The `craftflow:research` skill (loaded via SKILL_HINTS) provides synthesis guidance.
...
5. **Codebase Reality Check (MANDATORY for non-trivial work)** - Identify the exact files, modules, patterns, and integration points that support or constrain the plan. Do not finalize a non-trivial plan before comparing it against the current codebase and surfacing mismatches.
   ...
   - **ADR Prior-Art Check:** Before proposing a new cross-cutting convention, ledger, or
     process mechanism, search `docs/ai/decisions/` (if it exists in this project) for an
     existing `accepted`/`rejected`/`superseded` ADR on the same or an overlapping topic (grep
     the proposal's key nouns across `docs/ai/decisions/*.md`). If a matching ADR exists, treat
     it as a durable source of truth: do not re-propose an already-rejected alternative without
     new evidence, and do not silently duplicate an already-accepted mechanism. Surface any
     conflict explicitly under `Open Decisions` rather than reconciling it silently.
...
```
Skill(skill="craftflow:plan-review-gate")
```
...
- File paths exact? (+15)
<!-- CRAFTFLOW-M16: These factors are guidance only. Router CONTRACT RULE enforces CONFIDENCE<50 → NEEDS_CLARIFICATION. Scores 50-89 are self-assessed — not mechanically validated. -->
...
## Common Shortcuts (Anti-Rationalization)

| Shortcut | Why It Fails |
|----------|-------------|
| "I'll clarify the details during build" | Details left for build become hidden assumptions. Builder stops on ambiguity — surface it in OPEN_DECISIONS or return NEEDS_CLARIFICATION. |
| "Requirements seem clear enough" | "Seems clear" is not "verified against the repo." Read the actual files and confirm the real pattern before finalizing the plan. |
| "I'll use a Recommended Default instead of asking" | Recommended Defaults are unapproved. Material implementation choices require explicit user confirmation before BUILD starts. |
| "The plan is 90% complete — I'll note the gaps" | An incomplete plan is a failed plan. PLAN_CREATED requires empty OPEN_DECISIONS and DIFFERENCES_FROM_AGREEMENT present. |
...
Note: Craftflow internal skills such as `frontend-patterns` or `architecture-patterns` may be passed by the router via `SKILL_HINTS` when relevant. Do not assume they are preloaded.
