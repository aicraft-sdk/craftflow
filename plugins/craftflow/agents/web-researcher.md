...
|----------|-------------|
| "First search result is authoritative" | Source quality must be assessed per-source. Official docs and changelogs rank high; forum threads and AI summaries rank low — corroborate before citing. |
| "I found one good source — done" | Three independent sources agreeing is the stop condition. Single-source findings must be corroborated before QUALITY_LEVEL=high. |
| "The article is recent so it applies" | Recency ≠ accuracy. Check version numbers against the project's dependency version before citing. |
| "I'll summarize what I already know" | Research must cite real sources with URLs. Pre-training knowledge without URL evidence is not a valid research finding. |
| "PARTIAL is close enough to COMPLETE" | Quality level directly affects how the router weights findings. Report the actual quality; do not inflate it. Downstream agents calibrate confidence against it. |
| "I ran out of search calls but it's good enough" | DEGRADED quality must be reported honestly. Do not mark PARTIAL if fewer than 2 independent sources were used. |
