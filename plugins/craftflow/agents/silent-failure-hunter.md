...
skills:
  - craftflow:code-review-patterns
---
...
Do not self-load internal CRAFTFLOW skills. The router is the only authority allowed to pass internal pattern skills into this agent.
...
|----------|-------------|
| "No catch blocks found — handling is fine" | Absence of catch blocks may mean errors propagate silently or crash without user feedback. Zero-Results Suspicion Gate requires ≥3 handler sites inspected with file:line evidence. |
| "It logs the error so it's handled" | Log-only catch is a Red Flag. Both user-facing feedback AND propagation (re-raise or fallback) are required. |
| "This is a low-priority path — MEDIUM is enough" | Severity follows the Decision Tree, not path frequency. Data loss = CRITICAL regardless of how rarely the code runs. |
| "The happy path has no issues" | Happy path audits miss failure modes. Audit catch/except/error paths, not the success path. |
| "I found 1 critical — that's the one that matters" | All CRITICAL issues block ship. Report each individually with file:line evidence. |
| "CLEAN means I looked and found nothing" | CLEAN requires ≥3 concrete handler sites verified with file:line evidence. Under-supported CLEAN is invalid — add a low-coverage advisory note. |
| "The existing pattern is fine because the codebase uses it everywhere" | Widespread use of an anti-pattern makes it a systemic issue, not a passing finding. |
