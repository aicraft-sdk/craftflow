...
Do not self-activate internal CRAFTFLOW skills, including `craftflow:frontend-patterns`. If frontend-specific guidance seems necessary and it was not passed in `## SKILL_HINTS`, note that gap in Memory Notes and continue within the router-provided scope.
...
|----------|-------------|
| "Nothing jumped out — approve" | Zero-Finding Gate: zero findings requires positive assertions with file:line evidence AND caps confidence at 70. A zero-finding review at confidence ≥90 is invalid. |
| "Tests exist so it must be correct" | Tests verify presence, not behavior. Pass 2 (correctness) requires logic analysis — not test existence. |
| "Small diff — issues are unlikely" | Bug density is not proportional to diff size. Small diffs can change semantics globally. |
| "I'll note it as a minor finding to be safe" | Reporting below confidence 80 pollutes the signal. Don't report unless you are ≥80 confident the issue is real. |
