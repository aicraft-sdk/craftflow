...
Run a **read-only** pass — no edits, no agents that write. Use Grep/Glob directly to enumerate the source tree.
...
3. **Cross-reference near-duplicates.** Use Grep/Glob directly to find near-duplicate helpers by scanning for exported definitions with similar names — the goal is to surface same intent, different name helpers. Consolidate duplicates *before* deepening: two shallow copies merge into one, and only then is it worth asking whether that one should be deeper.
...
This skill ends at a chosen candidate + a deeper-interface proposal + a named seam. It does **not** edit code. The refactor is a behavior-preserving change and gets craftflow's full bar:
