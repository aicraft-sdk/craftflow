...
name: update
description: |
  Safe craftflow upgrade that preserves local modifications.
  Stashes diffs, pulls upstream, rebuilds cache, rebases patches.

  Use this skill when: updating craftflow, upgrading, pulling latest craftflow,
  syncing plugin, refreshing cache, or checking for new versions.

  Triggers: update craftflow, upgrade craftflow, pull craftflow, sync plugin, refresh craftflow,
...
CACHE_ROOT="$HOME/.claude/plugins/cache/craftflow/craftflow"
BACKUP_DIR="$HOME/.claude/plugins/cache/craftflow/_backup_$(date +%Y%m%d_%H%M%S)"
```
...
Extract the `craftflow@craftflow` entry. If missing → STOP: "craftflow is not installed. Install it first with `/install craftflow`."
...
Extract `craftflow.installLocation` → this is `MARKETPLACE_ROOT` (the local git clone of the craftflow repo).
The plugin source lives at `MARKETPLACE_ROOT/plugins/craftflow/`.
...
From `installed_plugins.json`, extract for the `craftflow@craftflow` entry:
...
AskUserQuestion(questions=[{
  "question": "Proceed with craftflow update?",
  "header": "Update",
  "options": [
...
```bash
MARKETPLACE_SOURCE="$MARKETPLACE_ROOT/plugins/craftflow"
...
entry = reg['plugins']['craftflow@craftflow'][0]
...
```bash
REL_PATH="${PATCH_FILE#$BACKUP_DIR/patches/}"    # e.g., skills/craftflow-router/SKILL.md.patch
TARGET_REL="${REL_PATH%.patch}"                    # e.g., skills/craftflow-router/SKILL.md
...
| Registry missing `craftflow@craftflow` | STOP — craftflow not installed |
...
| Multiple entries in `craftflow@craftflow` array | Use the first entry (primary installation) |
