...
CRAFTFLOW memory is a small, stable, versioned Markdown index plus durable workflow artifacts.
...
- Router-owned markers such as `[DEBUG-RESET: wf:{...}]` and
  `[craftflow-internal] memory_task_id` are read and respected by agents; agents do not invent
  replacements.
