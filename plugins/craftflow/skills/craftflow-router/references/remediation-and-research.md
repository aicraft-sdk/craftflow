...
```text
TaskCreate({
  subject: "CRAFTFLOW bug-investigator: Continue investigation",
...
8. Include `craftflow:research` in `## SKILL_HINTS` only when at least one research file exists.
...
```text
TaskCreate({
  subject: "CRAFTFLOW code-reviewer: Re-review after REM-FIX",
...
```text
TaskCreate({
  subject: "CRAFTFLOW silent-failure-hunter: Re-hunt after REM-FIX",
...
```text
TaskCreate({
  subject: "CRAFTFLOW integration-verifier: Re-verify after REM-FIX",
