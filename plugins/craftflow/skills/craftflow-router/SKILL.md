...
|-------|-----------------|
| component-builder | `STATUS`, `SUMMARY`, `CONFIDENCE`, `PHASE_ID`, `PHASE_STATUS`, `PHASE_EXIT_READY`, `CHECKPOINT_TYPE`, `PROOF_STATUS`, `INPUTS`, `EXPECTED_ARTIFACTS`, `TDD_RED_EXIT`, `TDD_GREEN_EXIT`, `SCENARIOS`, `ASSUMPTIONS`, `DECISIONS`, `BLOCKED_ITEMS`, `SKIPPED_ITEMS`, `SCOPE_INCREASES`, `BLOCKING`, `NEXT_ACTION`, `REMEDIATION_NEEDED`, `REQUIRES_REMEDIATION`, `REMEDIATION_REASON`, `MEMORY_NOTES` |
| bug-investigator | `STATUS`, `SUMMARY`, `VERIFICATION_RIGOR`, `CONFIDENCE`, `ROOT_CAUSE`, `TDD_RED_EXIT`, `TDD_GREEN_EXIT`, `VARIANTS_COVERED`, `BLAST_RADIUS_SCAN`, `SCENARIOS`, `ASSUMPTIONS`, `DECISIONS`, `BLOCKING`, `NEXT_ACTION`, `REMEDIATION_NEEDED`, `REQUIRES_REMEDIATION`, `REMEDIATION_REASON`, `NEEDS_EXTERNAL_RESEARCH`, `RESEARCH_REASON`, `MEMORY_NOTES` |
| planner | `STATUS`, `SUMMARY`, `PLAN_MODE`, `VERIFICATION_RIGOR`, `CONFIDENCE`, `PLAN_FILE`, `PHASES`, `RISKS_IDENTIFIED`, `SCENARIOS`, `ASSUMPTIONS`, `DECISIONS`, `OPEN_DECISIONS`, `DIFFERENCES_FROM_AGREEMENT`, `RECOMMENDED_DEFAULTS`, `ALTERNATIVES`, `DRAWBACKS`, `PROVABLE_PROPERTIES`, `BLOCKING`, `NEXT_ACTION`, `REMEDIATION_NEEDED`, `REQUIRES_REMEDIATION`, `REMEDIATION_REASON`, `GATE_PASSED`, `USER_INPUT_NEEDED`, `MEMORY_NOTES` |
...
`SUMMARY` is a single human-readable sentence describing what happened this
round — the router surfaces it directly in `status_history`/event-log
entries and memory distillation so a workflow's progress can be scanned
without re-parsing every structured field (borrowed from deepseek-harness's
Ralph-loop round handoff). It is additive to, not a replacement for, the
richer structured fields above (`SCENARIOS`, `ROOT_CAUSE`, `MEMORY_NOTES`,
etc.) — do not let a present `SUMMARY` substitute for the actual evidence
those fields carry.
...
5. Update `$PROJECT_ROOT/.craftflow/state/workflows/{workflow_uuid}.json` with:
   ...
   - the WRITE agent's `SUMMARY` field, into `results.{agent}.summary` and echoed into the
     matching `agent_completed` event-log entry as `"summary"` — gives a one-line round
     handoff scannable from `status_history`/the event log without re-parsing every
     structured field on every resume
...
- Updates `$PROJECT_ROOT/.craftflow/state/project/progress.md ## Completed` with a one-line summary of the finished workflow, via `craftflow_memory_merge.py` bullets-mode (section: `Completed`, `max_bullets: 20`, `archive: {"dir_rel": ".craftflow/state/project/archive", "section_slug": "completed", "month": "<current UTC YYYY-MM>"}`) — the same cap+archive mechanism already used for `patterns.md ## Common Gotchas`, reusing the existing `.craftflow/state/project/archive/completed-2026-08.md` archive file/slug established by a prior one-time rotation. Never omit `max_bullets`/`archive` for this destination; omitting it is exactly the gap that let this section regrow unbounded once already (a prior one-time manual rotation on 2026-08-19 archived it once, but no ongoing per-workflow cap was ever wired in afterward, so it silently regrew from ~25KB back to ~77KB by 2026-08-25 — six days, roughly 15 workflows).
- Caps project-tier `## Last Updated` in ALL THREE project memory files the same way, using `craftflow_memory_merge.py`'s `"unit": "entries"` mode (paragraph-shaped, not bullet-shaped — see the memory-finalize instruction sites for the full field contract): `max_bullets: 20` each, archived to `.craftflow/state/project/archive/` with per-file slugs `activecontext-last-updated`, `progress-last-updated`, `patterns-last-updated` respectively (distinct slugs required — all three files share the identical `## Last Updated` heading, so a shared slug would collide in the archive directory).
...
- **Known deferred gap (do not silently "fix" without a dedicated PLAN):** `activeContext.md ## Current Focus`, `activeContext.md ## Recent Changes`, and `progress.md ## Current Workflow` (project-tier) exhibit the same unbounded-append anti-pattern as `## Last Updated`/`## Completed` above and are NOT yet capped — `## Current Focus` in particular is typically the single largest contributor to `activeContext.md`'s size, well past `## Last Updated`. Fixing these requires a product decision (replace-in-place per their own single-value template contract vs. cap+archive like a history log), not just wiring the existing mechanism, and is out of scope here. `progress.md ## Current Workflow` already has a precedent archive file from one prior one-time manual rotation (`.craftflow/state/project/archive/current-workflow-history-2026-08.md`, slug `current-workflow-history`) that a future fix should resume, not replace.
...
- Never `git add` or `git commit` a plan artifact (`docs/plans/**`, any `PLAN_FILE`/`design_file` the planner produces, or a workflow's own `.craftflow/state/workflows/*.json`). Plans are working documents, not deliverables — `docs/plans/` is gitignored in full (see `.gitignore`). Their durable content belongs in the memory system (`## 13. Memory Finalization`: `learnings`/`patterns`/`verification` notes), not in git history. Recording `- Plan: {plan_file}` or `Plan saved: {plan_file}` in `activeContext.md` (a memory pointer to a local path) is unaffected by this rule and still required. If any dispatched agent stages or commits a plan file anyway, the router must `git restore --staged`/`git reset` that path before continuing — never let it ride along in the next commit.
